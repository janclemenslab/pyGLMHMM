# -*- coding: utf-8 -*-
"""
Created on Thu Feb  6 10:41:20 2020
...
def _trans_learning_fun(trans_w, trans_w_diag, stim, state_num, options):    
    ...
    trans_w_temp = np.zeros((num_states, num_total_bins))
    j = 0
    for i in range(0, num_states):
        if i == state_num:
           trans_w_temp[i, :] = trans_w_diag
        else:
           trans_w_temp[i, :] = trans_w[j * num_total_bins:(j + 1) * num_total_bins]
           j = j + 1           
    trans_w = trans_w_temp
    ...
    all_grad = np.delete(all_grad, np.arange(state_num * num_total_bins, (state_num + 1) * num_total_bins))
