    ...
    def _m_step_emission(self, new_stim):
        ...
        for i in range(0, self.num_states):
            if self.num_steps == 1:
                ...
                outweights = minimize(lambda x: _emit_multistep_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2]), order = 'F'), jac = True, method = 'L-BFGS-B', options = {'disp': None, 'maxcor': 100, 'ftol': 2.220446049250313e-09, 'gtol': 1e-05, 'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15000, 'iprint': 101, 'maxls': 25})
    ...
    def _m_step_transition(self, new_stim):
        ...
        for i in range(0, self.num_states):           
            if self.train_bins.size == 0:
                ...
                outweights = minimize(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, self.train_bins].T, (self.trans_w_.shape[1] * len(self.train_bins)), order = 'F'), jac = True, method = 'L-BFGS-B', options = {'disp': None, 'maxcor': 100, 'ftol': 2.220446049250313e-09, 'gtol': 1e-05, 'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15000, 'iprint': 101, 'maxls': 25})
