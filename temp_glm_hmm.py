...
from _minimize_LBFGS import _minimize_LBFGS
...
class GLMHMMEstimator(BaseEstimator):
    ...
    def fit(self, X, y, y_analog, emit_w, trans_w):   
        ...
        if do_init:
            self._initialize_parameters(X, emit_w, trans_w, self.random_state)
        ...
        for ind in range(0, self.max_iter):
            ...
            if self.evaluate == True:
                ...
                for i in range(0, self.num_states):
                    trans_w = copy.copy(self.trans_w_[i, np.setdiff1d(np.arange(self.num_states), i), :].T)
                    tmp_tgd[i] = _trans_learning_fun(np.reshape(trans_w, ((self.trans_w_.shape[1] - 1) * self.trans_w_.shape[2]), order = 'F'), self.trans_w_[i, i, :].T, new_stim, i, self.get_params())[0]
    ...
    def _initialize_parameters(self, X, emit_w, trans_w, random_state):       
    ...
    def _m_step_emission(self, new_stim):
        ...
        for i in range(0, self.num_states):
            if self.num_steps == 1:
                outweights = _minimize_LBFGS(lambda x: _emit_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2]), order = 'F'), lr = 1, max_iter = 1000, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 100, history_size = 10, out = True)
            else:
                outweights = minimize(lambda x: _emit_multistep_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2]), order = 'F'), jac = True, method = 'L-BFGS-B', options = {'disp': None, 'maxcor': 100, 'ftol': 2.220446049250313e-09, 'gtol': 1e-05, 'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15000, 'iprint': 101, 'maxls': 20})
    ...
    def _m_step_transition(self, new_stim):
        ...
        for i in range(0, self.num_states):
            ...
            if self.train_bins.size == 0:
                outweights = _minimize_LBFGS(lambda x: _trans_learning_fun(x, self.trans_w_[i, i, :].T, new_stim, i, self.get_params()), np.reshape(trans_w, ((self.trans_w_.shape[1] - 1) * self.trans_w_.shape[2]), order = 'F'), lr = 1, max_iter = 1000, tol = 1e-5, line_search = 'Wolfe', interpolate = True, max_ls = 100, history_size = 10, out = True)
                
                trans_w_temp = np.zeros((self.trans_w_.shape[1], self.trans_w_.shape[2]))
                k = 0
                for j in range(0, self.num_states):
                    if j == i:
                       trans_w_temp[j, :] = copy.copy(self.trans_w_[i, i, :])
                    else:
                       trans_w_temp[j, :] = outweights[k * self.trans_w_.shape[2]:(k + 1) * self.trans_w_.shape[2]]
                       k = k + 1
                self.trans_w_[i, :, :] = copy.copy(trans_w_temp)
                
                # self.trans_w_[i, :, :] = np.reshape(outweights['x'], (self.trans_w_.shape[2], self.trans_w_.shape[1]), order = 'F').T
            else:
                outweights = minimize(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, self.train_bins].T, (self.trans_w_.shape[1] * len(self.train_bins)), order = 'F'), jac = True, method = 'L-BFGS-B', options = {'disp': None, 'maxcor': 100, 'ftol': 2.220446049250313e-09, 'gtol': 1e-05, 'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15000, 'iprint': 101, 'maxls': 20})
