...
class GLMHMMEstimator(BaseEstimator):
    ...
    def _m_step_emission(self, new_stim):
        ...
        for i in range(0, self.num_states):
            if self.num_steps == 1:
                outweights = minimize(lambda x: _emit_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2], 1), order = 'F'), jac = True, method = 'BFGS', options = {'disp' : True})
            else:
                outweights = minimize(lambda x: _emit_multistep_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.emit_w_[i, :, :].T, (self.emit_w_.shape[1] * self.emit_w_.shape[2], 1), order = 'F'), jac = True, method = 'BFGS', options = {'disp' : True})
    ...
    def _m_step_transition(self, new_stim):
        ...
        for i in range(0, self.num_states):
            if self.train_bins.size == 0:
                outweights = minimize(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, :].T, (self.trans_w_.shape[1] * self.trans_w_.shape[2], 1), order = 'F'), jac = True, method = 'BFGS', options = {'disp' : True})
                self.trans_w_[i, :, :] = np.reshape(outweights['x'], (self.trans_w_.shape[2], self.trans_w_.shape[1]), order = 'F').T
            else:
                outweights = minimize(lambda x: _trans_learning_fun(x, new_stim, i, self.get_params()), np.reshape(self.trans_w_[i, :, self.train_bins].T, (self.trans_w_.shape[1] * len(self.train_bins), 1), order = 'F'), jac = True, method = 'BFGS', options = {'disp' : True})
